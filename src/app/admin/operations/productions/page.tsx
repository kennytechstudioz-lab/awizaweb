...
const DailyProductions: React.FC = () => {
  ...
  useEffect(() => {
    let typeParam = ''
    if (activeCategory === 'Fresh Eggs') {
      typeParam = `&productName[regex]=Fresh Eggs&productName[options]=i`
    } else if (activeCategory === 'Empty Sack') {
      typeParam = `&productName[regex]=Empty (Sack|Bag)&productName[options]=i`
    ...
    <>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 mb-4 card_body sharp">
        ...
        <div className="flex flex-1 gap-2 min-w-[300px]">
          {['All', 'Fresh Eggs', 'Empty Sack', 'Small Bag Manure', 'Big Bag Manure'].map(cat => (
            <button
              key={cat}
              type="button"
